...
this_dir="$(cd -L "$this_dir" && echo "$PWD")"
...
find_up()
{
    ...
    local t
    while path="$(cd -L "$path" && echo "$PWD")"; do
        t="$path/$name"
        if [ -e "$t" ]; then
            echo "$t"
            return 0
...
# config
c="$this_dir/$n.conf" && [ -r "$c" ] ||
    fatal 'no config file "%s"\n' "$c"

# script to patch
t='postinst' &&
r="$(find_up "$this_dir/$t")" && [ -w "$r" ] ||
    fatal 'no script to patch "%s"\n' "$t"
